...
class BinaryClassifier(Classifier):
    ...
    def _classify(self, y_proba: Tensor):
        ...
        #  Output should be a (N,) integer tensor.
        # ====== YOUR CODE: ======
        return torch.where(y_proba[:, self.positive_class] >= self.threshold, 1, 0).to(torch.int32)
        # ========================
...
def plot_decision_boundary_2d(
    ...
    x2_r = max(x[:,1])
    # grid
    x1 = torch.arange(x1_l, x1_r, dx)
    x2 = torch.arange(x2_l, x2_r, dx)
...
def select_roc_thresh(
    ...
    fpr, tpr, thresh = roc_curve(y.numpy(), y_proba[:, 1].detach().numpy())
